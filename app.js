...
// api key => 1448757832551236

const fetchAllSupperHero = async (searchText) => {
    let url = `https://www.superheroapi.com/api.php/1448757832551236/search/${searchText}`;
    try {
...
searchForm.search.addEventListener('keyup',() =>{
  if(searchForm.search.value.length > 1){
    fetchAllSupperHero(searchForm.search.value);
  } else{
    searchList.innerHTML = "";
  }
});
...
const showSuperheroDetails = (data) => {
  ...
  document.querySelector('.appearance').innerHTML =`
            ...
            <li>
                                    ...
                                    <span>${data[0].connections['group-affilation']}</span>
